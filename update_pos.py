...
file_path = 'C:\\xampp\\htdocs\\inventory\\php-inventory\\pages\\pos.php'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update the final redirect in POST handler
old_redirect = """    }
...
# 2. Add exit; after successful checkout redirect
old_checkout_success = """            $_SESSION['cart'] = [];
            set_flash('success', 'Transaction completed successfully. Order ID: #' . $saleId);
        }"""
new_checkout_success = """            $_SESSION['cart'] = [];
            set_flash('success', 'Transaction completed successfully. Order ID: #' . $saleId);
            redirect_to('pages/pos.php');
            exit;
        }"""
content = content.replace(old_checkout_success, new_checkout_success)

# 3. Add ID to main view and conditionally hide it
old_main_view = """<div class="flex h-[calc(100vh-140px)] -m-8 font-sans">
    <!-- Main Left side: POS List -->"""
new_main_view = """<!-- Main POS View -->
<div id="pos-main-view" class="<?php echo isset($_GET['checkout']) ? 'hidden' : 'flex'; ?> h-[calc(100vh-140px)] -m-8 font-sans">
    <!-- Main Left side: POS List -->"""
content = content.replace(old_main_view, new_main_view)

# 4. Remove the checkoutModal and insert the Checkout Page view
start_idx = content.find("<!-- Checkout Overlay -->")
...
<div id="pos-checkout-view" class="<?php echo isset($_GET['checkout']) ? 'flex' : 'hidden'; ?> h-[calc(100vh-140px)] -m-8 font-sans bg-white dark:bg-gray-900 text-black dark:text-white border-t border-black dark:border-gray-700">
    ...
    <div class="flex-1 flex flex-col p-12 items-center">
        ...
        <div class="w-full max-w-lg flex justify-end gap-4 mt-8">
...
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + checkout_view_html + content[end_idx:]

# 5. Fix Javascript functions
old_script_funcs = """    function openCheckoutModal() {
        closeCartModal();
...
new_script_funcs = """    function openCheckoutModal() {
        closeCartModal();
...
content = content.replace(old_script_funcs, new_script_funcs)

# 6. Check setPaymentMethod styling fixes
old_payment_js = """    function setPaymentMethod(method) {
...
new_payment_js = """    function setPaymentMethod(method) {
        document.getElementById('selectedPaymentMethod').value = method;
        if (method === 'CASH') {
...
content = content.replace(old_payment_js, new_payment_js)
